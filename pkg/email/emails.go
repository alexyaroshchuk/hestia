...
type Sender interface {
	Send(ctx context.Context, from, recipient string, subject, body string) error
}

// Service provides the main functionality for sending email.
type Service struct {
	from     string
	renderer Renderer
	sender   Sender
...
func NewService(from string, renderer Renderer, sender Sender) *Service {
	return &Service{
		from:     from,
		renderer: renderer,
		sender:   sender,
...
func (s *Service) Send(ctx context.Context, name string, recipient string, data any) error {
	var (
		sBuf bytes.Buffer
		bBuf bytes.Buffer
	)
